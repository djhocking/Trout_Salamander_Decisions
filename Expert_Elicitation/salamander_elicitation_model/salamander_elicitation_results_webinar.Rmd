...
**Expert elicitation** is a formal, structured process of obtaining expert opinion for specific questions.

**Expert opinion** is an expression of an expert’s belief based on knowledge and experience; it is an informed belief.  

* quantitative form = expert judgment
* qualitative form = expert knowledge

Successful elicitation entails capturing an expert’s true belief as well as her/his underlying rationale for this belief while minimizing cognitive bias to provide information that is otherwise unavailable.

There is no RIGHT answer in expert elicitation and variation within and among experts can reflect varies types of uncertainty (environmental variability across regions, ecological hypotheses, or variability in potential responses (parameter uncertainty)).

**Beware of Cognitive Traps**

1. Anchoring (opinions that are expressed first are used as a baseline)

2. Representativeness (most recent experiences are overly weighted)

3. Availability (unequal access to information to form judgments)

4. Consensus (unnecessary conformity to the group judgment without evidence)

5. Overconfidence (defending your opinion may result in unintentional increased confidence)

**Common Elicitation Questions**

1. what was your rational for these values?

2. what is the evidence to support this?

3. what are the highest and lowest possible values? and why?
...
```{r kable_q2b, echo=FALSE, warning=FALSE, message=FALSE,fig.width=7, fig.height=4}
...
region.order<- c("northern", "midatlantic", "southern")
q2b.tbl.1<-q2b.tbl %>%
  mutate(region =  factor(region, levels = region.order)) %>%
  arrange(species,region)

kable(q2b.tbl.1, digits=2)
ggplot(q2b.tbl.1, aes(x=species,y=mean,shape=factor(region),size=3))+geom_point()+ylab("mean number of occupied reaches")
```

**NEW:** Range-wide salamander occupancy had large potential ranges (upper and lower realistic values) with low confidence (i.e., <80%). As the potential range increases, confidence that the true value is within this range should also increase.

```{r kable_q2c, echo=FALSE, warning=FALSE, message=FALSE}
q2c.data<-subset(qall.data,question %in% c("2"))
q2c.data.expert<-subset(q2c.data,expert_ID %in% c(expert_no))
q2c.tbl<-as.data.frame(summarise(group_by(q2c.data,species,expert_ID),mean=mean_reaches,lowest=lower_reaches ,highest=upper_reaches,CI=confidence_reaches))
q2c.tbl.1<-select(q2c.tbl,species,mean,lowest,highest,CI)

range<-q2c.tbl.1$highest-q2c.tbl.1$lowest
q2c.tbl.2<-cbind(q2c.tbl.1,range)
colnames(q2c.tbl.2)<-c("species","mean","lowest","highest","CI","range")
plot(q2c.tbl.2$range,q2c.tbl.2$CI,pch=19,ylim=c(50,100),xlim=c(0,100),ylab="confidence that the true value is within the range",xlab="range = highest plausible value - lowest plausible value")
                     #order(q7b.tbl.1,decreasing="CI")

q2c.tbl.3<-q2c.tbl.2 %>% arrange(species,range,CI)
kable(q2c.tbl.3,format = "markdown")
```
...
q5.data.mean<-summarise(group_by(q5.data,species,flow_year_freq,stream_width_m),mean=mean(mean_reaches))
...
**NEW:** Expected number of occupied reaches (mean, median, SD, maximum, minimum, CV) across experts (N) compared to your the expected value when a resident brook trout population is present BY REGION.

```{r kable_q7b, echo=FALSE, warning=FALSE, message=FALSE}
q7b.data<-subset(qall.data,question %in% c("7"))
q7b.data.expert<-subset(q7b.data,expert_ID %in% c(expert_no))
q7b.tbl<-as.data.frame(summarise(group_by(q7b.data,species,region),mean=round(mean(mean_reaches),digits=2),median=round(median(mean_reaches),digits=2),SD=round(sd(mean_reaches),digits=2),max=max(mean_reaches),min=min(mean_reaches),CV=round(sd(mean_reaches)/mean(mean_reaches),digits=2),N=length(mean_reaches)))
q7b.tbl<-left_join(q7b.tbl,summarise(group_by(q7b.data.expert,species,region),your_value=round(mean(mean_reaches),digits=0)))
region.order<- c("northern", "midatlantic", "southern")
q7b.tbl.1<-q2b.tbl %>%
  mutate(region =  factor(region, levels = region.order)) %>%
  arrange(species,region)
kable(q7b.tbl.1, digits=2)
```

**NEW:** Salamander occupancy when a brook trout population was had large potential ranges (upper and lower realistic values) with low confidence (i.e., <80%). As the potential range increases, confidence that the true value is within this range should also increase.

```{r kable_q7c, echo=FALSE, warning=FALSE, message=FALSE}
q7c.data<-subset(qall.data,question %in% c("7"))
q7c.data.expert<-subset(q7c.data,expert_ID %in% c(expert_no))
q7c.tbl<-as.data.frame(summarise(group_by(q7c.data,species,expert_ID),mean=mean_reaches,lowest=lower_reaches ,highest=upper_reaches,CI=confidence_reaches))
q7c.tbl.1<-select(q7c.tbl,species,mean,lowest,highest,CI)
low.ci<-(q7c.tbl.1$CI<80)
low.ci["TRUE"]<-c(1)
low.ci["FALSE"]<-c(0)
#length(low.ci)
range<-q7c.tbl.1$highest-q7c.tbl.1$lowest
q7c.tbl.2<-cbind(q7c.tbl.1,range)
colnames(q7c.tbl.2)<-c("species","mean","lowest","highest","CI","range")
plot(q7c.tbl.2$range,q7c.tbl.2$CI,pch=19,ylim=c(50,100),xlim=c(0,100),ylab="confidence that the true value is within the range",xlab="range = highest plausible value - lowest plausible value")
                     #order(q7b.tbl.1,decreasing="CI")

q7c.tbl.3<-q7c.tbl.2 %>% arrange(species,range,CI)
kable(q7c.tbl.3, format="markdown")
```
...
psal_ecg<-ggplot(q8.data.ecg, aes(gpor_presence,col=factor(expert_ID)))+ geom_point(size=2,aes(y=mean_reaches),col="grey")+ geom_line(size=.75,aes(y=mean_reaches),q8.data.ecg)+ ylim(0, 100)+ theme(legend.position="none")+scale_colour_manual(values=rep("grey",length(unique(q8.data.ecg$expert_ID))))+ facet_grid(.~species, scales="free")+ geom_line(size=1.5,aes(y=mean,x=gpor_presence),q8.data.ecg.mean,col="black")+ ylab("elicited number of occupied reaches")+geom_line(size=1,aes(y=mean_reaches,x=gpor_presence),q8.data.ecg.expert,col="red")+geom_point(size=2,aes(y=mean_reaches,x=gpor_presence),q8.data.ecg.expert,col="red")+xlab("presence of gpor = 1")
...
psal_gd<-ggplot(q8.data.gd, aes(dfus_presence,col=factor(expert_ID)))+ geom_point(size=2,aes(y=mean_reaches),col="grey")+ geom_line(size=.75,aes(y=mean_reaches),q8.data.gd)+ ylim(0, 100)+ theme(legend.position="none")+scale_colour_manual(values=rep("grey",length(unique(q8.data.gd$expert_ID))))+ facet_grid(.~species, scales="free")+ geom_line(size=1.5,aes(y=mean,x=dfus_presence),q8.data.gd.mean,col="black")+ ylab("elicited number of occupied reaches")+geom_line(size=1,aes(y=mean_reaches,x=dfus_presence),q8.data.gd.expert,col="red")+geom_point(size=2,aes(y=mean_reaches,x=dfus_presence),q8.data.gd.expert,col="red")+xlab("presence of dfus = 1")
...
```{r kable_q2d, echo=FALSE, warning=FALSE, message=FALSE}
...
A. Is there strong support that species-specific relationships to the factors elicited in this survey would VARY among regions (southern, mid-atlantic, northern)? If so, please explain. 

B. Do experts have evidence that EBIS would NOT have similar relationships to factors in this survey compared to ECIR? If not, please explain.

C. Do experts have evidence that DFUS would NOT have similar relationships to factors in this survey compared to DCON? If not, please explain.
...
* No variation in center vs. edge of range effects on occupancy
...
1. Experts revise your responses based on webinar discussions, insights, and critisims.

2. Experts fill out any additional survey questions (i.e., network structure).

3. Data analysts predict catchment-level occupancy (mean, min, max, variance) for each species across the NE region (using SHEDS-ICE database of catchments). 

4. Data analysts compare predictions from expert judgement to empirical observations of presence/absence (i.e., model validation).
