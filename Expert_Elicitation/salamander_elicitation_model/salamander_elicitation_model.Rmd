...
```{r import data chuck, echo=FALSE, results='hide'}
# install libraries
library(knitr)
library(xtable)
...
```{r predictor ranges chuck, echo=FALSE}
...
intercept[sim]<-rnorm(no.simulations,mean=log(0.5/(1-0.5)),sd=0.1) # 0.5 average occupancy
b1[sim]<-rnorm(no.simulations,log(10),sd=0.1) # 5x more likely to occur for every 1 percent increase in forest cover
b2[sim]<-rnorm(no.simulations,log(0.90),sd=0.1) # 10x less likely to occur for every 1 sqkm increase in drainage area
}

# distribution of beta's
par(mfrow=c(3,2))
