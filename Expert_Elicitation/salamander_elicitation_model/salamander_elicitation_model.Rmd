...
*Expert results (with calculated betas)*

*Grand mean intercept (b0)*

Raw values from experts for grand mean intercept (b0):
```{r view expert b0, echo=FALSE }
# elicitation list of ranges
b0.list<-matrix(nrow=3,ncol=9,rep(NA,3*9))
b0.list<-q2.data[,1:3]
print(b0.list)
```

Calculate grand mean intercept (b0):
```{r calculate b0, echo=FALSE}
#q2.data
q2.data$logit.p<-log(q2.data$avg_pr_occ/(1-q2.data$avg_pr_occ))
b0.data<-list()
b0.data$nonlogit<-summarise(group_by(q2.data,species), mean=mean(avg_pr_occ), sd=sd(avg_pr_occ))
b0.data$logit<-summarise(group_by(q2.data,species), mean=mean(logit.p), sd=sd(logit.p))
b0.data<-data.frame(b0.data)
b0.data
no.species=3
no.rand=100
occ<-data.frame("DEFU"=integer(0),"GYPO"=integer(0),"EUBI"=integer(0))
for(species in 1:no.species){for(rand in 1:no.rand){
  occ[rand,species]<-1/(1+exp(-(rnorm(1,b0.data[species,"logit.mean"],b0.data[species,"logit.sd"]))))
}}
print(summary(occ))
```

*drainage area effect (b1)*

Raw values from experts for drainage area effect (b1):
```{r view expert b1, echo=FALSE }
# elicitation list of ranges
b1.list<-as.data.frame(q3.data)
b1.list.DEFU<-subset(b1.list,b1.list$species=="DEFU")
b1.list.GYPO<-subset(b1.list,b1.list$species=="GYPO")
b1.list.EUBI<-subset(b1.list,b1.list$species=="EUBI")
boxplot(b1.list.DEFU$pr_occ~b1.list.DEFU$drainagearea_sqkm)
plot(b1.list.DEFU$pr_occ~b1.list.DEFU$drainagearea_sqkm,type="b")

# regresssion
hist(q3.data$pr_occ)
q3.data$logit.p2<-log(q3.data$pr_occ/(1-q3.data$pr_occ))
head(q3.data)
library(dplyr)
str(q3.data)
q3.defu <- q3.data %>%
  dplyr::filter(species == "DEFU")

ggplot(q3.defu, aes(log(drainagearea_sqkm), logit.p2, colour = factor(expert_ID))) + geom_line()

lm1<-lmer(logit.p2 ~ log(drainagearea_sqkm) + (1|expert_ID), data = q3.defu)

summary(lm1)
plot(predict(lm1,q3.defu))
q3.defu$pred <- predict(lm1,q3.defu)
q3.defu$resid<- resid(lm1)
plot(lm1)
ggplot(q3.defu, aes(drainagearea_sqkm, resid, colour = factor(expert_ID))) + geom_point()
ggplot(q3.defu, aes(expert_ID, resid, fill = factor(expert_ID), group = factor(expert_ID))) + geom_boxplot()
ggplot(q3.defu, aes(pred, resid, colour = factor(expert_ID))) + geom_point()
  
library(ggplot2)
ggplot(q3.defu, aes(logit.p2, pred)) + geom_point()

for (i in 1:no.samples){for(area for 1:no.drainage)
  occ[i,expert]<-
}
```

Calculate drainage area effect (b1):
```{r calculate b1, echo=FALSE, results='hide'}
b1.data<-list()
b1.data$slope<-summarise(group_by(b1.list,species), mean=mean(slope,na.rm = TRUE), sd=sd(slope,na.rm = TRUE))
b1.data$logit.slope<-summarise(group_by(b1.list,species), mean=mean(logit.slope,na.rm = TRUE), sd=sd(logit.slope,na.rm = TRUE))

no.species=3
no.drainagearea=200
drainagearea.cov<-seq(min(q3.data$drainagearea_sqkm),max(q3.data$drainagearea_sqkm),length=no.drainagearea)

occ<-data.frame("DEFU"=integer(0),"GYPO"=integer(0),"EUBI"=integer(0))
for (species in 1:no.species){ for(i in 1:no.drainagearea){
  occ[i,species]<-1/(1+exp(-(b0.data[species,"logit.mean"]+b1.data$logit.slope[species,"mean"]*drainagearea.cov[i])))
}}
par(mfrow=c(1,1))
plot(occ[,1]~drainagearea.cov,col=1,type="l")
lines(occ[,2]~drainagearea.cov,col=2)
lines(occ[,3]~drainagearea.cov,col=3)
print(summary(occ))
```


*Stream temperature effect (b2)*

Raw values from experts:
```{r plot raw b2 results, echo=FALSE, results='hide',fig.width=5,fig.height=8}
q4.data
q4.data.DEFU<-subset(q4.data,q4.data$species=="DEFU")
q4.data.GYPO<-subset(q4.data,q4.data$species=="GYPO")
q4.data.EUBI<-subset(q4.data,q4.data$species=="EUBI")

q4.data.DEFU$pr_occ

par(mfrow=c(2,1))
plot(subset(q4.data.DEFU,q4.data.DEFU$expert_ID==1)$mean_summer_temp_C,subset(q4.data.DEFU,q4.data.DEFU$expert_ID==1)$pr_occ,ylim=c(0.5,1),type="l",col=1,xlab="b2, stream temp, C",ylab="prob of occupancy")
lines(subset(q4.data.DEFU,q4.data.DEFU$expert_ID==2)$mean_summer_temp_C,subset(q4.data.DEFU,q4.data.DEFU$expert_ID==2)$pr_occ,type="l",col=2)
lines(subset(q4.data.DEFU,q4.data.DEFU$expert_ID==3)$mean_summer_temp_C,subset(q4.data.DEFU,q4.data.DEFU$expert_ID==3)$pr_occ,type="l",col=3)
```

*Streamflow effect (b3)*
...
Raw values from experts:
```{r plot raw b4 results, echo=FALSE, results='hide',fig.width=5,fig.height=8}
q6.data
q6.data.DEFU<-subset(q6.data,q6.data$species=="DEFU")
q6.data.GYPO<-subset(q6.data,q6.data$species=="GYPO")
q6.data.EUBI<-subset(q6.data,q6.data$species=="EUBI")

q6.data.DEFU$pr_occ

par(mfrow=c(1,1))
plot(subset(q6.data.DEFU,q6.data.DEFU$expert_ID==1 & q6.data.DEFU$upland_percent_forest==0)$riparian30m_percent_forest,subset(q6.data.DEFU,q6.data.DEFU$expert_ID==1 & q6.data.DEFU$riparian30m_percent_forest==0)$pr_occ, ylim=c(0,1),type="l",col=1,xlab="b4, riparian forest cover",ylab="prob of occupancy")

lines(subset(q6.data.DEFU,q6.data.DEFU$expert_ID==1 & q6.data.DEFU$upland_percent_forest==50)$riparian30m_percent_forest,subset(q6.data.DEFU,q6.data.DEFU$expert_ID==1 & q6.data.DEFU$riparian30m_percent_forest==50)$pr_occ, type="l",col=2)
lines(subset(q6.data.DEFU,q6.data.DEFU$expert_ID==1 & q6.data.DEFU$upland_percent_forest==100)$riparian30m_percent_forest,subset(q6.data.DEFU,q6.data.DEFU$expert_ID==1 & q6.data.DEFU$riparian30m_percent_forest==100)$pr_occ, type="l",col=3)
legend("topleft",legend=c("0% upland forest", "50% upland forest", "100% upland forest"), text.col=c(1,2,3),cex=0.6)
...
Raw values from experts:
```{r plot raw fish results, echo=FALSE, results='hide',fig.width=5,fig.height=8}
q7.data
q7.data.DEFU<-subset(q7.data,q7.data$species=="DEFU")
q7.data.GYPO<-subset(q7.data,q7.data$species=="GYPO")
q7.data.EUBI<-subset(q7.data,q7.data$species=="EUBI")

q7.data.DEFU$pr_occ

par(mfrow=c(1,3))
plot(subset(q7.data,q7.data$expert_ID==1&q7.data$species=="DEFU")$trout_presence,subset(q7.data,q7.data$expert_ID==1&q7.data$species=="DEFU")$pr_occ, ylim=c(0,1),type="l",col=1,xlab="fish presence",ylab="prob of occupancy",main="DEFU")
lines(subset(q7.data,q7.data$expert_ID==2&q7.data$species=="DEFU")$trout_presence,subset(q7.data,q7.data$expert_ID==2&q7.data$species=="DEFU")$pr_occ,type="l",col=2)
lines(subset(q7.data,q7.data$expert_ID==3&q7.data$species=="DEFU")$trout_presence,subset(q7.data,q7.data$expert_ID==3&q7.data$species=="DEFU")$pr_occ,type="l",col=3)

plot(subset(q7.data,q7.data$expert_ID==1&q7.data$species=="GYPO")$trout_presence,subset(q7.data,q7.data$expert_ID==1&q7.data$species=="GYPO")$pr_occ, ylim=c(0,1),type="l",col=1,xlab="fish presence",ylab="prob of occupancy",main="GYPO")
lines(subset(q7.data,q7.data$expert_ID==2&q7.data$species=="GYPO")$trout_presence,subset(q7.data,q7.data$expert_ID==2&q7.data$species=="GYPO")$pr_occ,type="l",col=2)
lines(subset(q7.data,q7.data$expert_ID==3&q7.data$species=="GYPO")$trout_presence,subset(q7.data,q7.data$expert_ID==3&q7.data$species=="GYPO")$pr_occ,type="l",col=3)

plot(subset(q7.data,q7.data$expert_ID==1&q7.data$species=="EUBI")$trout_presence,subset(q7.data,q7.data$expert_ID==1&q7.data$species=="EUBI")$pr_occ, ylim=c(0,1),type="l",col=1,xlab="fish presence",ylab="prob of occupancy",main="EUBI")
lines(subset(q7.data,q7.data$expert_ID==2&q7.data$species=="EUBI")$trout_presence,subset(q7.data,q7.data$expert_ID==2&q7.data$species=="EUBI")$pr_occ,type="l",col=2)
lines(subset(q7.data,q7.data$expert_ID==3&q7.data$species=="EUBI")$trout_presence,subset(q7.data,q7.data$expert_ID==3&q7.data$species=="EUBI")$pr_occ,type="l",col=3)
```
