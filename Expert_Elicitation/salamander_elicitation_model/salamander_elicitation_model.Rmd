...
WE DID NOT USE 4-STEP: JUST ONE b/c too many questions and calcuated confidence based on diversity among experts. Martin et al. 2005 (Eco Apps) only elicited the mean, and calculated the mean and precision using mean values across experts (did not elicit lowest, highest and confidence). Use BBN figure (Example BBN for elicitation of variables, see Ban et al. 2015) to identify important conditional probabilities (i.e., riparian x upland, local x upstream) effects?
...
qall.drainage<- qall.data %>%
  dplyr::filter(question=="3" & expert_ID>="4")
p1<-ggplot(qall.drainage, aes(drainage,pr_occ,colour=factor(species), shape=factor(expert_ID), group=interaction(expert_ID,species))) + geom_point(size=3) + geom_line() 

qall.temp<- qall.data %>%
  dplyr::filter(question=="4"& expert_ID>="4")
p2<-ggplot(qall.temp, aes(temp,pr_occ,colour=factor(species), shape=factor(expert_ID), group=interaction(expert_ID,species))) + geom_point(size=3) + geom_line() 
...
p2<-ggplot(qall.trout, aes(trout,pr_occ,colour=factor(species), shape=factor(expert_ID), group=interaction(expert_ID,species))) + geom_point(size=3) + geom_line() 
...
qall.defu<-subset(qall.data,qall.data$species=="DEFU" & qall.data$expert_ID>=4 & qall.data$logit.p>=-99)
qall.gypo<-subset(qall.data,qall.data$species=="GYPO" & qall.data$expert_ID>=4 & qall.data$logit.p>=-99)
qall.eubi<-subset(qall.data,qall.data$species=="EUBI" & qall.data$expert_ID>=4 & qall.data$logit.p>=-99)
