...
# get featureids for the all basins 
...
df_huc_01 <- tbl_huc12 %>%
     ...
     dplyr::filter(substr(huc12, as.integer(1), as.integer(2)) == '01')
df_huc_01 <- dplyr::collect(df_huc_01)
head(df_huc_01)
length(unique(df_huc_01$HUC4))   # 10 HUC 4's
length(unique(df_huc_01$HUC8))  # 58 HUC 8's
length(unique(df_huc_01$HUC10))  # 395 HUC 10's
length(unique(df_huc_01$huc12))  # 1927 HUC 12's
length(df_huc_01$featureid)
featureids_01<-as.integer(df_huc_01$featureid)
# get covariates for the all featureids 
tbl_covariates_01 <- tbl(db, 'covariates')
tbl_covariates_01 <- dplyr::filter(tbl_covariates_01, featureid %in% featureids_01 ) 
#df_cov_long_01<-dplyr::collect(tbl_covariates_01) # this takes ~ 701pm-720
df_covariates_01<-tidyr::spread(df_cov_long_01,variable,value)
df_covariates_01<-left_join(df_covariates_01,df_huc_01)
summary(df_covariates_01)
df_covariates_m_01<-as.matrix((df_covariates_01))
df_covariates_m_01
str(df_covariates_m_01)
#write.csv(df_covariates_m_01,file="df_covariates_01.csv")
```
```{r import covariate data and create cov.df,echo=FALSE, results='hide', eval=FALSE}
# import saved covariate data from db
cov.data.01<-read.csv("/Users/katz-umass/github_folder/Trout_Salamander_Decisions/Expert_Elicitation/salamander_elicitation_model/df_covariates_01.csv",fill=TRUE,header=TRUE)

# replace NA's with -999
cov.data.01[is.na(cov.data.01)] <- -999

# select covariates to used in model
covariates<-cov.data.01 %>% 
  select(featureid,AreaSqKM,zone,riparian_distance_ft,ann_tmax_c,forest,jun_prcp_mm, jul_prcp_mm,aug_prcp_mm,impervious,slope_pcnt,elevation,huc12,HUC4,HUC10,HUC8)
str(covariates)

# subset local data (temp)
cov1<-subset(covariates,zone=="local" & ann_tmax_c > -999)
str(cov1) #83519 catchments
ptempl<-ggplot(cov1,aes(cov1$ann_tmax_c)) + geom_histogram(alpha=1,binwidth=.1) + xlim(8,25) + ggtitle("max annual temp")
pelevl<-ggplot(cov1,aes(cov1$elevation)) + geom_histogram(alpha=1,binwidth=5) + xlim(0,1000) + ggtitle("elevation")
pslopl<-ggplot(cov1,aes(cov1$slope_pcnt)) + geom_histogram(alpha=1,binwidth=1) + ggtitle("slope")
ggplot()+geom_point(aes(cov1$elevation,cov1$slope_pcnt))+ylim(0,100)+xlim(0,100)+xlab("local elevation")+ylab("local slope")

# subset upstream data (drainage area and precip)
cov2<-subset(covariates, zone=="upstream" & ann_tmax_c > -999)
flow<-c(cov2$jun_prcp_mm+cov2$jul_prcp_mm+cov2$aug_prcp_mm)/3*cov2$AreaSqKM
cov2$flow<-flow
str(cov2) #82934 catchments
pareau<-ggplot(cov2,aes(AreaSqKM)) + geom_histogram(alpha=1, binwidth=20) + ggtitle("drainage area")
pflowu<-ggplot(cov2,aes(flow)) + geom_histogram(alpha=1, binwidth=1) + ggtitle("precip x area")
pslopu<-ggplot(cov2,aes(slope_pcnt)) + geom_histogram(alpha=1, binwidth=1) + ggtitle("upstream slope")
pimpeu<-ggplot(cov2,aes(impervious)) + geom_histogram(alpha=1, binwidth=1) + ggtitle("upstream impervious")
ggplot()+geom_point(aes(cov2$AreaSqKM,cov2$slope_pcnt))+xlab("upstream area")+ylab("upstream slope")
...
pforer<-ggplot(cov3,aes(forest)) + geom_histogram(alpha=1, binwidth=1) + ggtitle("riparian forest")
pimper<-ggplot(cov3,aes(impervious)) + geom_histogram(alpha=1, binwidth=1) + ggtitle("riparian impervious")
ggplot()+geom_point(aes(cov3$forest,cov3$impervious))+ylim(0,100)+xlim(0,100)+xlab("percent forest in the riparian zone (30-m)")+ylab("percent impervious cover in the riparian zone (30-m)")

# view histograms
grid.arrange(ptempl,pelevl,pslopl)
grid.arrange(pareau,pflowu,pslopu)
grid.arrange(pforer,pimper)

# save subset data as cov.df 
# 83519 catchments
cov.df<-data.frame(featureid=cov1$featureid,l.slop=cov1$slope_pcnt,l.temp=cov1$ann_tmax_c,l.temp2=cov1$ann_tmax_c^2,l.fore=cov1$forest,u.impe=cov2$impervious,u.area=cov2$AreaSqKM,u.flow=cov2$flow,r.fore=cov3$forest,r.impe=cov3$impervious)
str(cov.df)
...
#str(q1sum.data)
```
```{r plot expert results,            echo=FALSE, results='hide', eval=FALSE}
...
e26="#FFCC99" # peach 
e27="#FFCC99" # peach 
e.color<-c(e10,e11,e12,e13,e14,e15,e16,e17,e18,e19,e20,e21,e22,e23,e24,e25,e26,e27)
...
Removed expert plots of expert data - see salamander_elicitation_results_webinar.RMD

```{r plot expert results for EACH expert,  echo=FALSE, results='hide'}
# remove NA's
qall.data<-subset(qall2.data,mean_reaches>=0)
str(qall.data)
...
q2.tbl<-as.data.frame(summarise(group_by(q2.data,species),mean=round(mean(mean_reaches),digits=2),median=round(median(mean_reaches),digits=2),sd=round(sd(mean_reaches),digits=2),max=max(mean_reaches),min=min(mean_reaches),cv=round(sd(mean_reaches)/mean(mean_reaches),digits=2),no.experts=length(mean_reaches),your_value=round(mean(mean_reaches),digits=0)))
...
q3.data.mean<-summarise(group_by(q3.data,species,drainage_area_sqkm),mean=mean(mean_reaches))
plogd<-ggplot(q3.data, aes(log(drainage_area_sqkm),col=factor(expert_ID))) + geom_point(size=0,aes(y=mean_reaches)) + geom_line(size=.75,aes(y=mean_reaches),q3.data) + ylim(0, 100) + theme(legend.position="none") + scale_colour_manual(values=rep("grey",length(unique(q3.data$expert_ID)))) + facet_grid(.~species, scales="free") + geom_line(size=1,aes(y=mean,x=log(drainage_area_sqkm)),q3.data.mean,col="black") + ylab("elicited number of occupie reaches")
plogd
# add real scale for log-drainage values (0 to 200sqkm)
# loop through and add expert-value in red 
```
...
ptemp<-ggplot(q4.data, aes(summer_temp_C,col=factor(expert_ID)))+ geom_point(size=0,aes(y=mean_reaches))+ geom_line(size=.75,aes(y=mean_reaches),q4.data)+ ylim(0, 100)+ theme(legend.position="none")+ scale_colour_manual(values=rep("grey",length(unique(q4.data$expert_ID))))+ facet_grid(.~species, scales="free")+ geom_line(size=1,aes(y=mean,x=summer_temp_C),q4.data.mean,col="black")+ ylab("elicited number of occupie reaches")
ptemp
# loop through and add expert-value in red 
```
...
```{r calculate betas (glmer0 - no re), echo=FALSE, results='hide', eval=FALSE}
...
```{r calculate betas (glmer1 - rand int + slops), echo=FALSE, results='hide', eval=FALSE}
...
# use startgazer() to summarize results of a lmer (latex format)
```{r plot predicted values,          echo=FALSE, results='hide', eval=FALSE}
library("effects")
par(mfrow=c(2,2))
...
```
```{r export out_betas,               echo=FALSE, results='hide', eval=FALSE}
...
```
```{r export cov_ranges,              echo=FALSE, results='hide', eval=FALSE}
...
```
```{r plot betas,                     echo=FALSE,result='hide', eval=FALSE}
str(betas.coef)
